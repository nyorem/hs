...
-- same as fs.hs but equipped with Maybe
...
fsTo name (Folder folderName items, bs) =
    let (ls, maybeItem) = break (nameIs name) items in
        case maybeItem of
            [] -> Nothing
...
fsNewFile newItem (Folder folderName items, bs) = Just (Folder folderName (newItem:items), bs)
